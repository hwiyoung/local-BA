...
def solve_direct_georeferencing(images, epsg=5186):
    ...
    ori = rpy_to_opk(np.array(camera.reference.rotation[::-1]))
...
def solve_lba_esti_div(images, epsg=5186):
    start_time = time.time()
    ...
    chunk.cameras[-1].reference.location = \
        Metashape.CoordinateSystem.transform(point=chunk.cameras[-1].reference.location,
    ...
    # n most recent (n-1) images
    ## TODO: using for loop, set accuracy corresponding to no. images to process
...
def solve_lba_esti_uni(images, epsg=5186):
    return 0, 0, 0, 0
